...
export default function DashLotoutRubro() {
  ...
  const {usuario, panels, activePanel} = useAuth();
  console.log(usuario)
  ...
  console.log(panels)
      ...
      const vistas = [
        ...
        { name: "empresas", path: "/rubro/dashboard/empresas", icon: <BsBuildings /> },
        { name: "Solicitudes de empresa", path: "/rubro/dashboard/empresasSoli", icon: <FaBuildingShield /> },
        { name: "Clientes", path: "/rubro/dashboard/clientes", icon: <BsFillPeopleFill /> },
        { name: "Tecnicos", path: "/rubro/dashboard/tecnicos", icon: <MdEngineering /> },
        { name: "Configuración roles", path: "/rubro/dashboard/configuracion", icon: <IoIosSettings /> },
      ];
    ...
    <div className={`flex h-screen overflow-hidden transition-colors duration-300 ${darkMode ? "dark bg-slate-900" : ""}`}>
      ...
      <aside
        className={`
          ...
          bg-primary dark:bg-slate-800 dark:border-slate-700
      ...
      >
        {/* Logo Area */}
        <div className="h-20 flex items-center justify-between px-6  dark:border-slate-700">
          <div className="flex items-center gap-3 dark:text-primary text-white">
          ...
          {vistas.map((vista) => {
            ...
            return (
              <Link
                ...
                `}
              >
                <span className={`text-lg ${isActive ? "text-white" : "text-text dark:text-gray-400 group-hover:text-warning dark:group-hover:text-white"}`}>
                    {vista.icon}
                </span>
      ...
      {/* CONTENIDO PRINCIPAL */}
      <div className="flex flex-col flex-1 min-w-0 bg-[#ebebea] dark:bg-slate-900 scroll-smooth"> {/* min-w-0 evita overflow en flex children */}
